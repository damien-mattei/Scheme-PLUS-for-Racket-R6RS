...
;; /Applications/Racket\ v8.13/bin/plt-r6rs --install overload.scm
...
(define (check-arguments pred-list args)
  ...
  (if (= (length pred-list) (length args))
	...
	;; replace andmap with every in Guile, R6RS
  ...
  (syntax-rules ()
    ...
    ((_ proc orig-proc)
     ...
     (begin
       ...
       ;; cause errors in R6RS:
       ;;(import
	;;(require ;; error: require: not at module level or top level
	 ;;(rename-in
	  ;;racket/base
	 ;;(rnrs base (6))
	  ;;(proc orig-proc)))
...
;;   (overload-existing-operator + vector-append (vector? vector?))
;;   (define rv (+ (vector 1 2 3) (vector 4 5 6 7)))
;;   (display rv) (newline)
